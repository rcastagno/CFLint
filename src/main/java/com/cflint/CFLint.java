...
public class CFLint implements IErrorReporter {
	...
	public CFLint(CFLintConfig configFile) throws IOException {
		...
		try {
			allowedExtensions.addAll(Arrays.asList(ResourceBundle
	...
	@Deprecated
	public CFLint(ConfigRuntime configuration,
			final CFLintScanner... bugsScanners) {
		super();
		...
		for (final CFLintScanner scanner : bugsScanners) {
			...
			if (configuration != null) {
				PluginInfoRule ruleInfo = configuration.getRuleByClass(scanner
						.getClass());
				if (ruleInfo != null) {
	...
	public void scan(final String folder) {
		...
		if (showProgress) {
			final ProgressMonitorListener progressMonitorListener = new ProgressMonitorListener(
					progressMonitorName);
	...
	private int prescan(File folderOrFile, int counter,
	...
	public void scan(final File folderOrFile) {
		if (folderOrFile.isDirectory()) {
			...
			try {
				...
				/*
				 * bugs.add(new
				 * BugInfo.BugInfoBuilder().setMessageCode("FILE_ERROR")
				 * .setFilename
				 * (folderOrFile.getAbsolutePath()).setMessage(e.getMessage
				 * ()).setSeverity("ERROR") .build());
				 */
				if (logError) {
					System.out.println("Logging Error: " + FILE_ERROR);
					fireCFLintException(e, FILE_ERROR,
							folderOrFile.getAbsolutePath(), null, null, null,
							null);
				}
			}
		}
	...
	public void process(final String src, final String filename)
		...
		if (elements.size() == 0 && src.contains("component")) {
			...
			final CFScriptStatement scriptStatement = cfmlParser
					.parseScript(src);
	...
	public void processStack(final List<Element> elements, final String space,
			final String filename, final CFIdentifier functionName)
			throws ParseException, IOException {
		for (final Element elem : elements) {
			final Context context = new Context(filename, elem, functionName,
					inAssignment, handler);
			process(elem, space, context);
		}
	}

	public void processStack(final List<Element> elements, final String space,
			final Context context) throws ParseException, IOException {
		for (final Element elem : elements) {
	...
	private void process(final Element elem, final String space, Context context)
		...
		try {
			...
			if (elem.getName().equalsIgnoreCase("cfset")
					|| elem.getName().equalsIgnoreCase("cfif")
					|| elem.getName().equalsIgnoreCase("cfelseif")
					|| elem.getName().equalsIgnoreCase("cfreturn")) {
				final int elemLine = elem.getSource().getRow(elem.getBegin());
				final int elemColumn = elem.getSource().getColumn(
						elem.getBegin());
				...
				if (m.matches()) {
					...
					try {
						...
						// cfmlParser.parseScript(cfscript + ";");
						// process(scriptStatement, context.getFilename(), elem,
						// context.getFunctionName());
						// }else{
						final CFExpression expression = cfmlParser
								.parseCFExpression(cfscript, this);

						if (expression == null) {
							throw new NullPointerException(
									"expression is null, parsing error");
						}
						process(expression, context.getFilename(), elem,
								context.getFunctionName());
						// }
					} catch (final Exception npe) {
						final int line = elem.getSource().getRow(
								elem.getBegin());
						final int column = elem.getSource().getColumn(
								elem.getBegin());
						if (!quiet) {
							System.err.println("Error in: "
									+ shortSource(elem.getSource(), line)
									+ " @ " + line + ":");
							if (verbose) {
						...
						/*
						 * bugs.add(new
						 * BugInfo.BugInfoBuilder().setLine(elemLine
						 * ).setColumn(elemColumn + column)
						 * .setMessageCode("PARSE_ERROR"
						 * ).setSeverity("ERROR").setExpression(m.group(1))
						 * .setFilename
						 * (filename).setFunction(functionName).setMessage
						 * ("Unable to parse").build());
						 */
						// if (logError) {
						// System.out.println("Logging Error: " + PARSE_ERROR);
						// fireCFLintException(npe,PARSE_ERROR,context.getFilename(),elemLine,elemColumn
						// + column,context.getFunctionName(),m.group(1));
						// }
					}
				}
				...
				final CFScriptStatement scriptStatement = cfmlParser
						.parseScript(cfscript);

				process(scriptStatement, context.getFilename(), elem,
						context.getFunctionName());
				// } else if (elem.getName().equalsIgnoreCase("cfoutput")) {
				// final Element parent = CFTool.getNamedParent(elem,
				// "cfoutput");
				// if (parent != null && parent.getAttributeValue("query") !=
				// null
				// && parent.getAttributeValue("group") == null) {
				// final int line = elem.getSource().getRow(elem.getBegin());
				// final int column =
				// elem.getSource().getColumn(elem.getBegin());
				// bugs.add(new
			...
			if (elem.getName().equalsIgnoreCase("cffunction")) {
				...
				if (elem.getAttributeValue("value") != null) {
					// final CFScriptStatement scriptStatement =
					// cfmlParser.parseScript(elem.getAttributeValue("value") +
					// ";");
					// process(scriptStatement, context.getFilename(), elem,
					// context.getFunctionName());
	...
	private List<CFLintStructureListener> getStructureListeners(
			final List<CFLintScanner> extensions) {
	...
	private String shortSource(final Source source, final int line) {
		...
		try {
			final BufferedReader sr = new BufferedReader(new StringReader(
					source.toString()));
	...
	private void process(final CFScriptStatement expression,
			final String filename, final Element elem,
			final CFIdentifier functionName) {
		process(expression, filename, elem, functionName.getName());
	}

	private void process(final CFScriptStatement expression,
			final String filename, final Element elem, String functionName) {
		final Context context = new Context(filename, elem, functionName,
				inAssignment, handler);
		...
		if (expression instanceof CFCompoundStatement) {
			for (final CFScriptStatement statement : ((CFCompoundStatement) expression)
					.getStatements()) {
				process(statement, filename, elem, functionName);
			}
		} else if (expression instanceof CFExpressionStatement) {
			process(((CFExpressionStatement) expression).getExpression(),
					filename, elem, functionName);
		} else if (expression instanceof CFCompDeclStatement) {
			// process the component declaration
			process(((CFCompDeclStatement) expression).getBody(), filename,
					elem, functionName);
			...
			process(((CFForStatement) expression).getInit(), filename, elem,
					functionName);
			process(((CFForStatement) expression).getCond(), filename, elem,
					functionName);
			process(((CFForStatement) expression).getNext(), filename, elem,
					functionName);
			process(((CFForStatement) expression).getBody(), filename, elem,
					functionName);
		} else if (expression instanceof CFForInStatement) {
			process(((CFForInStatement) expression).getVariable(), filename,
					elem, functionName);
			process(((CFForInStatement) expression).getStructure(), filename,
					elem, functionName);
			process(((CFForInStatement) expression).getBody(), filename, elem,
					functionName);
			...
			// TODO replace with cfreturn.getExpression() when next cfmlparser
			// is released
			Field f;
			try {
	...
	private void process(final CFExpression expression, final String filename,
			final Element elem, final String functionName) {
		final Context context = new Context(filename, elem, functionName,
				inAssignment, handler);
		context.setInComponent(inComponent);
		...
		if (expression instanceof CFUnaryExpression) {
			process(((CFUnaryExpression) expression).getSub(), filename, elem,
					functionName);
		} else if (expression instanceof CFNestedExpression) {
			process(((CFNestedExpression) expression).getSub(), filename, elem,
					functionName);
		} else if (expression instanceof CFAssignmentExpression) {
			inAssignment = true;
			process(((CFAssignmentExpression) expression).getLeft(), filename,
					elem, functionName);
			inAssignment = false;
			process(((CFAssignmentExpression) expression).getRight(), filename,
					elem, functionName);
		} else if (expression instanceof CFBinaryExpression) {
			process(((CFBinaryExpression) expression).getLeft(), filename,
					elem, functionName);
			process(((CFBinaryExpression) expression).getRight(), filename,
					elem, functionName);
			...
			process(((CFVarDeclExpression) expression).getInit(), filename,
					elem, functionName);
			...
			// if(expression.getToken().getType() ==
			// CFSCRIPTLexer.STRING_LITERAL){
			// for(String subExprStr : splitHash(expression.Decompile(0))){
			// try{
			// final CFExpression subExpression =
			// cfmlParser.parseCFExpression(subExprStr,this);
			// if (expression == null) {
			// throw new
			// NullPointerException("expression is null, parsing error");
			// }
			// process(subExpression, filename, elem,functionName);
			// } catch (final Exception npe) {
			// final int line = elem.getSource().getRow(elem.getBegin());
			...
			// if (!quiet) {
			// System.err.println("Error in: " + shortSource(elem.getSource(),
			// line) + " @ " + line + ":");
			// if (verbose) {
			// npe.printStackTrace(System.err);
			// }
			...
			for (CFExpression expr : ((CFStringExpression) expression)
					.getSubExpressions()) {
				process(expr, filename, elem, functionName);
			}
		} else {
	...
	protected void reportRule(final Element elem, final Object expression,
			final Context context, final CFLintScanner plugin, String msg) {
		final String[] exceptionmsg = (msg != null ? msg : "").split(":");
		final String msgcode = exceptionmsg[0].trim();
		final String nameVar = exceptionmsg.length > 1 ? exceptionmsg[1].trim()
				: null;
		reportRule(elem, expression, context, plugin, new ContextMessage(
				msgcode, nameVar));
	}
	...
	public static Element getPreviousSibling(Element element) {
		...
		if (element.getParentElement() != null) {
			List<Element> parentElements = element.getParentElement()
					.getChildElements();
			...
			List<Element> parentElements = element.getSource()
					.getChildElements();
			int idx = parentElements.indexOf(element);
			if (idx > 0) {
				return parentElements.get(idx - 1);
	...
	protected void reportRule(final Element elem, final Object expression,
			final Context context, final CFLintScanner plugin,
			ContextMessage msg) {
		...
		if (ruleInfo == null) {
			throw new NullPointerException("Rule not found for "
					+ plugin.getClass().getSimpleName());
		}
		final PluginMessage msgInfo = ruleInfo.getMessageByCode(msgcode);
		if (configuration == null) {
			throw new NullPointerException("Message definition not found for ["
					+ msgcode + "] in " + plugin.getClass().getSimpleName());
		}
		BugInfoBuilder bldr = new BugInfo.BugInfoBuilder()
				.setMessageCode(msgcode).setVariable(nameVar)
		...
		if (msgInfo != null) {
			...
			System.err.println("Message code: " + msgcode
	...
	public void reportError(final IntStream arg0,
			final RecognitionException arg1, final BitSet arg2) {
		// TODO Auto-generated method stub
		final String file = currentFile == null ? "" : currentFile + "\r\n";
		System.out.println(file + "-------" + arg0);
	...
	public void addScanProgressListener(
			final ScanProgressListener scanProgressListener) {
	...
	public void addExceptionListener(
			final CFLintExceptionListener exceptionListener) {
		exceptionListeners.add(exceptionListener);
	}

	protected void fireCFLintException(final Exception e,
		...
		for (final CFLintExceptionListener p : exceptionListeners) {
			p.exceptionOccurred(e, messageCode, filename, line, column,
	...
	public void syntaxError(Recognizer<?, ?> recognizer,
		...
		if (recognizer instanceof Parser
				&& ((Parser) recognizer)
						.isExpectedToken(CFSCRIPTParser.SEMICOLON)) {
			bugs.add(new BugInfo.BugInfoBuilder()
					.setMessageCode("MISSING_SEMI")
					.setFilename(file)
					.setMessage(
							"End of statement(;) expected instead of "
									+ expression).setSeverity("ERROR")
					.setExpression(expression).setLine(line)
					.setColumn(charPositionInLine).build());

		} else {

			fireCFLintException(e, PARSE_ERROR, file, line, charPositionInLine,
					"", msg);
		}
	}
	...
	public void reportError(org.antlr.v4.runtime.RecognitionException re) {
		final String file = currentFile == null ? "" : currentFile + "\r\n";
		System.out.println(file + "----RecognitionException ---"
				+ re.getCtx().getSourceInterval().a + " : "
